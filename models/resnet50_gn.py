# ================================================================
# FILE: models/resnet50_gn.py
# ResNet50 with Group Normalization baseline for CIFAR-10/CIFAR-100
# ================================================================
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes: int, planes: int, stride: int = 1, num_groups: int = 32):
        ...
        self.norm3 = GN(planes * self.expansion, num_groups)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != planes * self.expansion:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, planes * self.expansion, kernel_size=1, stride=stride, bias=False),
                GN(planes * self.expansion, num_groups),
            )
...
def ResNet50_GN(num_classes: int = 100, num_groups: int = 32):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, num_groups=num_groups)
