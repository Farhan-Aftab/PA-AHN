# ================================================================
# FILE 1: bn_normalization.py
# Training / evaluation / logging for Batch Normalization baseline
...
# Note:
# - Entropy is logged as 0.0 because BN has no adaptive branch routing.
...
CFG.experiment_name = (
    f"bn_baseline_{CFG.dataset_name.lower()}_{CFG.model_type}_"
...
def get_baseline_norm_indicator() -> Dict[str, float]:
    return {"BN": 1.0, "LN": 0.0, "IN": 0.0, "GN": 0.0}
...
def main():
    ...
    if CFG.model_type == "resnet18":
        model = ResNet18_BN(num_classes=CFG.num_classes).to(device)
    elif CFG.model_type == "resnet50":
        model = ResNet50_BN(num_classes=CFG.num_classes).to(device)
    else:
    ...
    plot_curve(epochs, train_loss_history, f"BN Training Loss - {CFG.experiment_name}", "Epoch", "Loss", os.path.join(paths["results"], f"{CFG.experiment_name}_train_loss.png"))
    plot_curve(epochs, train_acc_history, f"BN Training Accuracy - {CFG.experiment_name}", "Epoch", "Accuracy (%)", os.path.join(paths["results"], f"{CFG.experiment_name}_train_accuracy.png"))
    plot_curve(epochs, test_acc_history, f"BN Test Accuracy - {CFG.experiment_name}", "Epoch", "Accuracy (%)", os.path.join(paths["results"], f"{CFG.experiment_name}_test_accuracy.png"))
    plot_curve(epochs, entropy_history, f"BN Entropy Indicator - {CFG.experiment_name}", "Epoch", "Entropy", os.path.join(paths["results"], f"{CFG.experiment_name}_entropy.png"))
    plot_curve(epochs, epoch_time_history, f"BN Training Time per Epoch - {CFG.experiment_name}", "Epoch", "Time (sec)", os.path.join(paths["results"], f"{CFG.experiment_name}_epoch_time.png"))
    plot_curve(epochs, avg_batch_time_history, f"BN Average Batch Time - {CFG.experiment_name}", "Epoch", "Time (sec)", os.path.join(paths["results"], f"{CFG.experiment_name}_batch_time.png"))
    plot_curve(epochs, gpu_memory_history, f"BN GPU Memory Usage - {CFG.experiment_name}", "Epoch", "Memory (MB)", os.path.join(paths["results"], f"{CFG.experiment_name}_gpu_memory.png"))
    plot_norm_indicator_bars(all_epoch_averages, os.path.join(paths["results"], f"{CFG.experiment_name}_norm_indicators.png"), f"BN Normalization Indicator Across Epochs - {CFG.experiment_name}")

    final_lines = [
        "Training finished.",
        f"BN Parameter count: {total_trainable_params}",
        f"BN Best test accuracy: {best_test_acc:.2f}% at epoch {best_epoch}",
        f"BN Final test accuracy: {test_acc_history[-1]:.2f}%" if len(test_acc_history) > 0 else "Final test accuracy: N/A",
        f"BN Average epoch time: {np.mean(epoch_time_history):.2f}s" if len(epoch_time_history) > 0 else "Average epoch time: N/A",
        f"BN Total training time: {total_training_time_sec:.2f}s",
        f"BN Average batch time: {np.mean(avg_batch_time_history):.4f}s" if len(avg_batch_time_history) > 0 else "Average batch time: N/A",
        f"BN Peak GPU memory: {peak_gpu_memory_mb:.2f} MB",
        f"BN Metrics saved to: {metrics_csv}",
        f"BN Summary saved to: {summary_csv}",
        f"BN Indicator weights saved to: {weights_csv}",
    ]
