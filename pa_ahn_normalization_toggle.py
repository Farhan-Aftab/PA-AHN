# ================================================================
# FILE 1: pa_ahn_normalization_toggle.py
...
from models.resnet18_pa_ahn_toggle import ResNet18_AHN
from models.resnet50_pa_ahn_toggle import ResNet50_AHN
...
class Config:
    ...
    # Ablation switch:
    # True  = use prior-anchored routing: final_logits = base_logits + residual_scale * residual_logits
    # False = remove prior anchor:       final_logits = residual_logits
    ...
    # True  = use L-Stab:    total_loss = task_loss + lambda * entropy_loss
    # False = remove L-Stab: total_loss = task_loss
    use_lstab: bool = True
...
# Experiment naming
prior_tag = "with_prior_anchor" if CFG.use_prior_anchor else "without_prior_anchor"
lstab_tag = "with_lstab" if CFG.use_lstab else "without_lstab"
CFG.experiment_name = f"pa_ahn_toggle{prior_tag}_{lstab_tag}_{CFG.dataset_name.lower()}_{CFG.model_type}_batchsize_{CFG.batch_size}_lr_{CFG.lr}_wd_{CFG.weight_decay}_{CFG.num_epochs}epochs"
...
def main():
    ...
    if CFG.model_type == "resnet18":
        model = ResNet18_AHN(num_classes=CFG.num_classes, temperature=temperature, use_prior_anchor=CFG.use_prior_anchor).to(device)
    elif CFG.model_type == "resnet50":
        model = ResNet50_AHN(num_classes=CFG.num_classes, temperature=temperature, use_prior_anchor=CFG.use_prior_anchor).to(device)
    else:
        raise ValueError("Invalid model_type")

    # ------------------------------------------------------------
    # Initialize the same safe prior in every AHN layer.
    # If use_prior_anchor=False, these values are still stored/logged
    # for comparability, but they are not added to final routing logits.
    # ------------------------------------------------------------
    ...
    for epoch in range(1, CFG.num_epochs + 1):
        ...
        for inputs, labels in train_loader:
            ...
            # If CFG.use_lstab=False, get_lambda_lstab() returns 0.0,
            # so this becomes exactly: total_loss = task_loss.
    ...
    summary_metrics = {
        ...
        "Batch_Size": [CFG.batch_size],
        "Use_Prior_Anchor": [CFG.use_prior_anchor],
        "Use_LStab": [CFG.use_lstab],
        "Lambda": [lambda_history[-1] if len(lambda_history) > 0 else 0.0],
        "Configured_Lambda_Max": [CFG.lstab_lambda_max],
